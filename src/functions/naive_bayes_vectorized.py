...
class SnowflakeSentimentVectorized:
    ...
    def predict_all(
        ...
        test_reviews["PREDICTION"] = test_reviews["REVIEW"].apply(
            self._predict, args=(probabilities, p_0, p_4, vocabulary)
        )
        ...
        training_results = pd.DataFrame(
            {
                ...
                "PROB_0": [
                    token_prob[0][word] if token_prob[0][word] else default_prob
                    for word in vocabulary
                ],
                "PROB_4": [
                    token_prob[4][word] if token_prob[4][word] else default_prob
                    for word in vocabulary
                ],
            }
        )
